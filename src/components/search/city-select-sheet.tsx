...
import { DOMESTIC_AIRPORTS, INTERNATIONAL_AIRPORTS, DOMESTIC_POPULAR, INTL_POPULAR, type AirportEntry } from "@/data/airports";
...
function searchAirports(airports: AirportEntry[], q: string): AirportEntry[] {
  ...
  return airports.filter(a=>a.iata.includes(upper)||a.city.includes(q)||a.city.toLowerCase().includes(lower)||a.name.includes(q));
}
...
export function CitySelectSheet({ open, onClose, onSelect, selectedIata }: CitySelectSheetProps) {
  ...
  const handleSelect = useCallback((airport: AirportEntry) => { onSelect(airport); onClose(); }, [onSelect, onClose]);
      ...
      <div className="bg-[#1ab0a6] px-3 pb-0 flex-shrink-0" style={{paddingTop:"max(env(safe-area-inset-top),12px)"}}>
        <div className="flex items-center gap-2 py-2">
          ...
          <div className="flex-1 flex items-center bg-white rounded-md overflow-hidden">
            ...
            {query && <button onClick={()=>setQuery("")} className="px-2 py-1"><X className="w-4 h-4 text-gray-400" /></button>}
          </div>
        </div>
        <div className="flex mt-1">
          {(["domestic","intl"] as const).map((t,i)=>(
            <button key={t} onClick={()=>{setTab(t);setQuery("");}}
              className={cn("flex-1 py-2.5 text-sm font-medium border-b-2 transition-colors",tab===t?"text-white border-white":"text-white/60 border-transparent")}>
              {i===0?"国内":"国际/中国港澳台"}
            </button>
          ))}
        </div>
        ...
        <div ref={listRef} className="flex-1 overflow-y-auto overscroll-contain">
          {isSearching ? (
            searchResults.length===0
              ...
              : searchResults.map(a=><AirportRow key={a.iata} airport={a} selected={a.iata===selectedIata} onSelect={handleSelect}/>)
          ) : (
            <>
              ...
              <div className="bg-white mb-2">
                ...
                <div className="p-3 grid grid-cols-4 gap-2">
                  {popularAirports.map(a=>(
                    <button key={a.iata} onClick={()=>handleSelect(a)}
                      className={cn("py-2 px-1 text-sm rounded border text-center transition-all active:scale-95",
                        selectedIata===a.iata?"bg-[#1ab0a6] text-white border-[#1ab0a6] font-medium":"bg-white text-gray-700 border-gray-200 hover:border-[#1ab0a6] hover:text-[#1ab0a6]")}>
                      {a.city}
                    </button>
                  ))}
                </div>
              ...
              <div className="bg-white mb-2">
                {grouped.map(({letter,list})=>(
                  <div key={letter} ref={el=>{if(el)sectionRefs.current.set(letter,el)}}>
                    ...
                    {list.map(a=><AirportRow key={a.iata} airport={a} selected={a.iata===selectedIata} onSelect={handleSelect}/>)}
                  </div>
                ))}
              </div>
            </>
        ...
        {!isSearching && (
          <div className="absolute right-0 top-0 bottom-0 w-7 flex flex-col items-center justify-center gap-0 py-2 z-10">
            {ALPHABET.map(letter=>{
              ...
              return <button key={letter} onClick={()=>avail&&scrollToLetter(letter)}
                className={cn("w-5 h-5 text-[10px] font-bold flex items-center justify-center rounded-full transition-colors",
                  avail?"text-[#1ab0a6] active:bg-[#1ab0a6] active:text-white":"text-gray-300 cursor-default")}>{letter}</button>;
            })}
          </div>
        )}
      </div>
    ...
    <button onClick={()=>onSelect(airport)}
      ...
      <div className="flex flex-col">
        <span className={cn("text-sm font-medium",selected?"text-[#1ab0a6]":"text-gray-800")}>
          {airport.city}<span className="ml-2 text-xs font-mono text-gray-400">{airport.iata}</span>
        </span>
        <span className="text-xs text-gray-400 mt-0.5">
          {airport.name}{airport.country!=="中国"&&<span className="ml-1 text-gray-300">· {airport.country}</span>}
        </span>
      </div>
